...
fn bestmile_task_02() -> Element {
    Element::UserDefined(
        r#"
                \item{\textbf{Automated infrastructure deployments} with Atlantis and \textbf{GitOps}
...
fn bestmile_task_07() -> Element {
    Element::UserDefined(
        r#"
                \item{\textbf{Administered VPN bastion} hosts on Linux with bash scripts,
                \textbf{AWS-Packer}, and \textbf{Ansible}, automating configuration management and
                improving secure access protocols.}
    "#
        .to_string(),
    )
}
