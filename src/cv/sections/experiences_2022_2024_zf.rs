...
fn zf_task_02() -> Element {
    Element::UserDefined(
        r#"
                \item {Implements the \textbf{Mend Renovate} bot for Cloud Platform Engineering
                teams, ensuring applications, libraries, and modules are \textbf{updated to the
                latest stable versions}, thereby maintaining software stability and security.}
    "#
        .to_string(),
    )
}
...
fn zf_task_06() -> Element {
    Element::UserDefined(
        r#"
                \item {Revises internal \textbf{documentation} using the \textbf{Divio
                Documentation framework}, improving the usability and technical clarity of internal
                tools, thereby \textbf{improving the onboarding process} for new developers.}
    "#
        .to_string(),
    )
}
