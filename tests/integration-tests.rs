...
mod file_system_integration_tests {
    ...
    fn test_directory_creation_and_cleanup() {
        ...
        let nested_path = test_env.temp_dir.path().join("deep").join("nested").join("path");
...
mod command_execution_tests {
    ...
    fn test_nonexistent_command() {
        let result = Command::new("definitely_does_not_exist_command_12345")
            .output();
...
mod performance_tests {
    ...
    fn test_file_creation_performance() {
        ...
        for i in 0..100 {
            let file_path = test_env.temp_dir.path().join(format!("perf_test_{}.txt", i));
...
mod benchmark_tests {
    ...
    fn bench_file_creation(b: &mut test::Bencher) {
        ...
        b.iter(|| {
            let file_path = test_env.temp_dir.path().join(format!("bench_{}.txt", counter));
